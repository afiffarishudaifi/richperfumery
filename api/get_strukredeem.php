...
// $id_kasir = 11711;
$query_kasir = "SELECT ks.no_faktur, ks.tanggal_faktur, ks.id_pelanggan, ks.total_tagihan, ks.total_potongan, ks.ongkos_kirim,ks.uang_muka, mm.nama as nama_metodebayar, ks.total_metodebayar, mm2.nama as nama_metodebayar2, ks.total_metodebayar2, ks.metodebayar, ks.metodebayar2, ks.carabayar, ks.total_redeem FROM tbl_kasir ks LEFT JOIN m_metode as mm ON ks.metodebayar=mm.id LEFT JOIN m_metode as mm2 ON ks.metodebayar2=mm2.id WHERE id_kasir='$id_kasir'";
$result_kasir = mysqli_query($connect,$query_kasir);

$arraydata = array();
if ($result_kasir->num_rows>0) {

	$query_produk = "SELECT tbp.id, tbp.nama, tbkdp.jumlah, tbs.satuan_satuan, tbkdp.harga, tbkdp.total, tbkdp.poin, 
	tbkdp.total_poin
	...
	while ($baris = mysqli_fetch_assoc($result_kasir)) {
		...
		$total_redeem 	= $baris['total_redeem']; 
		...
		/*$arraydata = array(
			...
			"total_tagihan" => format_angka($total_tagihan),
...
*/
		$arraydata = array(
			...
			"no_member" 		=> $no_member,
			"tanggal_faktur" 	=> date("d-m-Y", strtotime($baris['tanggal_faktur'])),
			"total_poin" 		=> format_angka($total_redeem),
			"total_qty"			=> isset(mysqli_fetch_assoc($r_qty)['total_qty']) ? 
								   count(mysqli_fetch_assoc($r_qty)['total_qty']):0,
			"nama_metodebayar" 	=> "Redeem",
			"total_metodebayar" => 0,
			"status_metode" 	=> 10,
			"detail_produk" 	=> $produk_array,			
		);
