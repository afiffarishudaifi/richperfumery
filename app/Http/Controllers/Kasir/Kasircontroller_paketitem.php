...
class Kasircontroller_paketitem extends Controller
{
    //
    ...
    $term = trim($request->q);
        ...
        $barangs = DB::SELECT("SELECT mp.id as produk_id,
          ...
          CASE WHEN mp2.poin IS NULL THEN 0 ELSE mp2.poin END AS produk_poin, '' AS produk_gambar, 
          CASE WHEN mpm.harga IS NULL THEN 0 ELSE mpm.harga END as harga_promo
        FROM m_produk as mp
        LEFT JOIN (
    ...
    public function simpan(Request $request){
      ...
      $tabel_promo          = $request->get('tabel_promo');
      $tabel_nominal_promo  = $request->get('tabel_nominal_promo');
      $tabel_status_promo   = $request->get('tabel_status_promo');
      //$produk_cetak = array();
      $produk_cetak = "";
      ...
      if($id_kasir == ''){
        ...
        for($i=0;$i<count($tabel_id);$i++){
          ...
          $produk['promo']        = $tabel_promo[$i];
          $produk['nominal_promo']= $tabel_nominal_promo[$i];
          $produk['status_promo'] = $tabel_status_promo[$i];
      ...
      if($d_data->count() > 0){
        foreach($d_data->get() as $d){
            $arr['produk'][] = array('id' => $d->id_kasir_detail_produk,
                            ...
                            'status'        => "1",
                            'promo'         => $d->promo,
                            'nominal_promo' => $d->nominal_promo,
                            'status_promo'  => $d->status_promo,
