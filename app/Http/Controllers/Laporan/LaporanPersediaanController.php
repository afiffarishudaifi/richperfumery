...
class LaporanPersediaanController extends Controller
{
    ...
    public function cetaklaporan_persediaan(Request $request){
		...
		if ($request->get('jenis') == '2') {
			$data['data']	= DB::table("view_log_stock")
			->when($request->get('gudang') != 0, function($query) use($request){
				return $query->where('id_barang', $request['barang']);
			})
			->when($request->get('gudang') != 'all', function($query) use($request){
				return $query->where('id_ref_gudang', $request['gudang']);
			})
			->selectRaw('id,id_barang,barang_kode,id_ref_gudang,id_satuan,tanggal,unit_masuk as jumlah_masuk,unit_keluar as jumlah_keluar,status,nama_barang as barang_nama,nama_ref_gudang as gudang_nama,nama_satuan as satuan_satuan, "" as jumlah_stokawal, "" as jumlah_terakhir')
			->orderBy('tanggal', 'ASC')->get()->toArray();
			if($request->get('gudang') == 'all'){
				$data['gudang']	 = 'Semua Gudang';
			}else{
				$d_gudang = DB::table('ref_gudang')->where('id',$request->get('gudang'))->select(DB::Raw('id as id_gudang,nama as nama_gudang'))->first();
				$data['gudang']	 = $d_gudang->nama_gudang;
			}
			$data['kasir']   = Auth::user()->name;
			$html = view('admin.laporan.cetak_laporan_persediaan_detail')->with('data',$data);
    ...
    public function excel($d_gudang,$d_tanggalAwal, $d_tanggalAkhir, $d_barang){
		...
		return Excel::download(new LaporanViewExport($data), 'Laporan_persediaan_'.$nama_gudang.'_'.$tanggal.'-'.$tanggal2.'(Rekap).xlsx');
	...
	public function excel_detail($d_gudang, $d_barang, $jenis){
		$req['gudang'] = $d_gudang;
		$req['barang'] = $d_barang;
		if($d_gudang=='all'){
			$nama_gudang = 'Semua Gudang';
		}else{
			$nama_gudang = DB::table('ref_gudang')->where('id',$d_gudang)->first()->nama;
		}
		$data = $this->hasil_detail($req);
		return Excel::download(new LaporanViewExport($data), 'Laporan_persediaan_'.$nama_gudang.'(Detail).xlsx');
	}

	public function cetak_hasil_detail($d_gudang, $d_barang, $jenis){
		$req['gudang'] = $d_gudang;
		$req['barang'] = $d_barang;
		$data = $this->hasil_detail($req);
		return view('admin.laporan.hasil_laporan',compact('data'));
	...
	function hasil_detail($request)
	{
		$tabel = DB::table("view_log_stock")
		->when($request['barang'] != 0, function($query) use($request){
			return $query->where('id_barang', $request['barang']);
		})
		->when($request['gudang'] != 'all', function($query) use($request){
			return $query->where('id_ref_gudang', $request['gudang']);
		...
		if (count($tabel) > 0) {
			foreach ($tabel as $key => $value) {
				$count_stok_akhir += $value->unit_masuk;
				$res[] = [
					'NO.' => $no++,
					'TANGGAL' 				=> $value->tanggal,
					'KODE BARANG' 			=> $value->barang_kode,
					'NAMA PARFURM'			=> $value->nama_barang,
					'NAMA GUDANG'			=> $value->nama_ref_gudang,
					'STOK AWAL'				=> "",
					'STOK MASUK'			=> format_angka($value->unit_masuk).' '.$value->nama_satuan,
					'STOK KELUAR'			=> format_angka($value->unit_keluar).' '.$value->nama_satuan,
					'STOK AKHIR'			=> format_angka($count_stok_akhir).' '.$value->nama_satuan
				];
			}
		}else{
			$res = [
