...
class CetakActivity : AppCompatActivity(),
    ...
    /** SETTING PRINTER BLUETOOTH*/
    var BILL = ""
    private  var listData : MutableList<ResponseKasir> = mutableListOf()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        presenter =
            PrintPresenter(this, api)
        presenter.getDataKasir("$id_kasir")
        /** PRINT BLUETOOTH*/
        ...
        mScan.setOnClickListener {
            ...
            if (mBluetoothAdapter == null) {
                ...
                if (!mBluetoothAdapter!!.isEnabled) {
                    ...
                    ListPairedDevices()
                    val connectIntent = Intent(this@CetakActivity,
...
//            }
    ...
    override fun showDataList(data: ResponseKasir) {
...
//            toast("Barangnya ${data.detailProduk}")
            for (i in data.detailProduk!!.indices) {
                BILL += String.format(
                    ...
                    "@" + "${data.detailProduk[i].harga}",
                    "${data.detailProduk[i].total}\n"
                )
            }
        }else{
                toast("Produk kosong");
        }
        if (data.detailBarang != null){
//            toast("Barangnya ${data.detailBarang}")
            for (i in data.detailBarang!!.indices) {
                BILL += String.format(
                    "%1$1s %2$5s %3$10s %4$10s",
                    "${data.detailBarang[i].namaBarang}" + "\n",
                    "${data.detailBarang[i].jmlBarang}x",
                    "@" + "${data.detailBarang[i].hargaBarang}",
                    "${data.detailBarang[i].totalBarang}\n"
                )
            }
        }else{
//            toast("Barangnya Kosong")
        }
        ...
        val textUM= utilRight.format(data.uangMuka)
        val textOngkir= utilRight.format(data.ongkosKirim)
        val textPotongan= utilRight.format(data.totalPotongan)
        val textTagihan= utilRight.format(data.totalTagihan)
        val textStatusMetode= data.statusMetodebayar
//        val textSubtotal= utilRight.format(data.totalSubtotal)

        BILL += "Total QTY      $textQTY"
        //BILL += "Subtotal       $textSubtotal"
        BILL += "Uang Muka      $textUM"
        BILL += "Ongkos Kirim   $textOngkir"
        BILL += "Total Potongan $textPotongan"
        BILL += "Total Tagihan  $textTagihan"
        BILL += "--------------------------------\n"
        BILL += "${data.namaMetodebayar} : ${data.totalMetodebayar}\n"
        if (textStatusMetode!="2"){
                BILL += "${data.namaMetodebayar2} : ${data.totalMetodebayar2}\n"
        }
        BILL += "================================\n"
        BILL += "$thanksText\n"
        BILL += "\n\n "
