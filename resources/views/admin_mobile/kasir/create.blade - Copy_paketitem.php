...
<section class="content">
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="box box-primary">
        ...
        <!-- /.box-header -->
        <div class="box-body">
          <form id="regForm" role="form" class="form_belanja" action="{{url('kasir_simpan')}}" method="post">
                    ...
                    <div class="row">
                      <div class="col-md-6">
                        <div class="form-group">
                          ...
                          <input class="form-control" type="text" name="tanggal" placeholder="Tanggal" value="{{($data['data']['tanggal'] == '') ? date('d-m-Y'):$data['data']['tanggal']}}" required readonly />
                        </div>
                        <div class="form-group">
                      ...
                      <div class="col-md-6">
                        ...
                        <div class="form-group">
                          ...
                          <input type="text"  name="nomor" class="form-control" value="{{($data['data']['nomor'] == '') ? $data['no_auto']:$data['data']['nomor']}}" readonly="">
                        </div>
                        <div class="form-group">
  ...
  $("#btn_tambah").click(function(){
      ...
      $('[name=popup_produk_sebelum]').val('');
      //PROMO
      $('[name=harga_promo]').val('0');
      $('[name=popup_promo]').val('TIDAK').trigger('change');
      $(`#wrapper-promo`).hide()
  ...
  $("[name=popup_jumlah]").on('keyup', function(){
    ...
    // PROMO
    var harga_promo = $(`[name=popup_hargapromo]`).val();
    if (harga_promo > 0 && jumlah > 1){
      $(`#wrapper-promo`).show()
    }else{
      $(`#wrapper-promo`).hide()
    }
  ...
  $("[name=popup_produk]").on('change', function(){
    ...
    $("[name=popup_hargapromo]").val(d.harga_promo);
    if(d.harga_promo == 0){
      $('[name=popup_promo]').val('TIDAK').trigger('change');
      $(`#wrapper-promo`).hide()
    }else{
      if($("[name=popup_jumlah]").val() > 1){
        $('[name=popup_promo]').val('TIDAK').trigger('change');
        $(`#wrapper-promo`).show()
      }
    }
...
$("[name=gudang]").on('change',function(){
  ...
  $("#btn_popup_simpan").click(function(){
    ...
    //PROMO
    if(status == '1'){
      var pilih_promo = $(`[name=popup_promo] :selected`).val();
      var promo       = $(`[name=popup_hargapromo]`).val();
      var status_promo= 0;
      var total_lama  = '';
      ...
      if(promo > 0){
        ...
        var pilih_promo  = 'TIDAK';
      }
      if ((pilih_promo) && pilih_promo == 'YA') {
        if (promo > 0 || isNumeric(promo)) {
          var total_promo = parseFloat(jumlah * promo);
              total       = parseFloat(jumlah * harga) - parseFloat(total_promo);
              total_lama  = '<br><s>'+accounting.formatMoney(parseFloat(harga*jumlah))+'</s>';
        }
      }else{
        promo = 0
      }
    ...
    if(id_data == ''){
      ...
      table_barang.row.add(['<div><center></center></div>',
            ...
            '<div id="list_total'+tb_no+'" class="text-right">'+accounting.formatMoney(total)+total_lama+'</div>',
            ...
            '<input type="hidden" name="tabel_promo[]" id="tabel_promo'+tb_no+'" value="'+pilih_promo+'">'+
            '<input type="hidden" name="tabel_nominal_promo[]" id="tabel_nominal_promo'+tb_no+'" value="'+promo+'">'+
            '<input type="hidden" name="tabel_status_promo[]" id="tabel_status_promo'+tb_no+'" value="'+status_promo+'">'+
      ...
      table_barang.row.add(['<div><center></center></div>',
            ...
            '<input type="hidden" name="tabel_poin[]" id="tabel_poin'+tb_no+'" value="'+poin+'">'+
            '<input type="hidden" name="tabel_status[]" id="tabel_status'+tb_no+'" value="'+status+'">'+
            '<input type="hidden" name="tabel_idproduk_sebelum[]" id="tabel_idproduk_sebelum'+tb_no+'" value="">']).draw(false);
      }
    }else{
        if(status == '1'){
          ...
          $("#list_total"+id_data).text(accounting.formatMoney(total)+total_lama);
          ...
          //promo
          $("#tabel_promo"+id_data).val(pilih_promo);
          $("#tabel_nominal_promo"+id_data).val(promo);
          $("#tabel_status_promo"+id_data).val(status_promo);
        }else{
  ...
  function edit_table(id){
    ...
    //PROMO
    var promo           = $("#tabel_promo"+id).val();
    var nominal_promo   = $("#tabel_nominal_promo"+id).val();
    var status_promo    = $("#tabel_status_promo"+id).val();
    if (promo == 'YA') {
      $(`#wrapper-promo`).show()
    }else{
      if(jumlah > 1){
    ...
    $("[name=popup_promo]").val(promo).trigger('change');
    $("[name=popup_hargapromo]").val(nominal_promo);
        ...
        success: function(respon){
          if(respon.produk.length > 0){
            for(i in respon.produk){
              var get_total_lama = 0;
              if(respon.produk[i].status_promo == 1){
                if(respon.produk[i].promo == 'YA'){
                  get_total_lama = '<br><s>'+accounting.formatMoney(respon.produk[i].harga*respon.produk[i].jumlah)+'</s>';
                }
              }
              table_barang.row.add(['<div><center></center></div>',
                ...
                '<div id="list_total'+respon.produk[i].id+'" class="text-right">'+accounting.formatMoney(respon.produk[i].total)+get_total_lama+'</div>',
                ...
                '<input type="hidden" name="tabel_promo[]" id="tabel_promo'+respon.produk[i].id+'" value="'+respon.produk[i].promo+'">'+
                '<input type="hidden" name="tabel_nominal_promo[]" id="tabel_nominal_promo'+respon.produk[i].id+'" value="'+respon.produk[i].nominal_promo+'">'+
                '<input type="hidden" name="tabel_status_promo[]" id="tabel_status_promo'+respon.produk[i].id+'" value="'+respon.produk[i].status_promo+'">'+
          ...
          if(respon.barang.length > 0){
            for(i in respon.barang){
              ...
              table_barang.row.add(['<div><center></center></div>',
                ...
                '<input type="hidden" name="tabel_idproduk_sebelum[]" id="tabel_idproduk_sebelum'+respon.produk[i].id+'" value="">'
                ]).draw(false);
            }
