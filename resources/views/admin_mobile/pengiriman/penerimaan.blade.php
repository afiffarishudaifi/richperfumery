    ...
    $(document).on("click","#btn_acc",function() {
      ...
      if(confirm("Apakah yakin untuk menerima retur?")){
        $.ajax({
          ...
          type : "POST",
          data : {'id_gudang_pusat':id_gudang_pusat,'id_satuan':id_satuan,'id_gudang_outlet':id_gudang_outlet,'jumlah':jumlah,'id_barang':id_barang,'id':id, 'tanggal':tanggal, '_token' : $('meta[name=csrf-token]').attr('content')},
...
$(".btn_popup_simpan_baru").on('click',function(){
  ...
  //console.log(status+"/"+tanggal+"/"+jumlah_terima);
