<?php $hal = "pembelianretur"; ?>
@extends('layouts.admin.master')
@section('title', 'Retur Pembelian')

@section('css')
<!-- DataTables -->
<link rel="stylesheet" href="{{asset('public/admin/bower_components/datatables.net-bs/css/dataTables.bootstrap.min.css')}}">

<style>
    .example-modal .modal {
      position: relative;
      top: auto;
      bottom: auto;
      right: auto;
      left: auto;
      display: block;
      z-index: 1;
    }

    .example-modal .modal {
      background: transparent !important;
    }

  </style>

@endsection


@section('content')
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1>
    Retur Pembelian
    <!-- <small>Data barang</small> -->
  </h1>
</section>
<!-- Main content -->
<section class="content">
  <div class="row">
    <div class="col-xs-12">

      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Data Retur Pembelian</h3>
          <div class="box-tools pull-right">
            <button type="button" class="btn btn-box-tool" data-toggle="collapse" title="Search" data-target="#form-search"> <i class="fa fa-search"></i> Search</button>
          </div>
        </div>
        <div class="box-body collapse" id="form-search">
          <form class="form-horizontal form-tanggal-search" method="POST" autocomplete="off">
            <div class="col-md-6">
              <div class="form-group">
                <label class="col-md-2">Tanggal</label>
                <div class="col-md-5">
                  <input type="text" name="search_tanggal" class="form-control" value="{{date('d-m-Y')}}">
                </div>
                <div class="col-md-4">
                  <a href="javascript:;" style="margin-bottom:20px;" class="card-body-title btn-search-tanggal"><button class="btn btn-success" type="button"><i class="fa fa-search"></i> Search</button></a>
                  <a href="javascript:;" style="margin-bottom:20px;" class="card-body-title btn-reset-tanggal"><button class="btn btn-warning" type="button"><i class="fa fa-undo"></i> Reset</button></a>
                </div>
              </div>
            </div>
          </form>
      </div>
        <!-- <a href="javascript:;" style="margin-bottom:20px;margin-left:10px;" class="card-body-title btn_tambah" id="btn_tambah"><button class="btn btn-primary"><i class="fa fa-plus-square-o"></i> Tambah</button></a> -->
        <?php echo $data['tombol_create'] ?>
        <!-- /.box-header -->
        <div class="box-body table-responsive">
          <table id="datatable1" class="table table-bordered table-striped">
            <thead>
              <tr>
                <th style="width:3%">No #</th>
                <th style="width:13%">Kode Retur</th>
                <th style="width:20%">Nama Barang</th>
                <th style="width:15%">Gudang</th>
                <th style="width:15%">Supplier</th>
                <th style="width:15%">Tanggal</th>
                <th style="width:10%">Jumlah</th>
                <th style="width:10%">Action</th>
              </tr>
            </thead>
            <tbody>
            </tbody>
          </table>
        </div>
        <!-- /.box-body -->
      </div>
      <!-- /.box -->
    </div>
    <!-- /.col -->
  </div>
  <!-- /.row -->
</section>
<!-- /.content -->
@include('admin.pembelian.form_retur')
@include('admin.pembelian.detail_retur')
@endsection


@section('js')
<!-- DataTables -->
<script src="{{asset('public/admin/bower_components/datatables.net/js/jquery.dataTables.min.js')}}"></script>
<script src="{{asset('public/admin/bower_components/datatables.net-bs/js/dataTables.bootstrap.min.js')}}"></script>
<script src="{{asset('public/admin/bower_components/select2/dist/js/select2.full.min.js')}}"></script>

<link rel="stylesheet" type="text/css" href="{{asset('public/js/daterangepicker/daterangepicker.css')}}" />
<script type="text/javascript" src="{{asset('public/js/daterangepicker/moment.min.js')}}"></script>
<script type="text/javascript" src="{{asset('public/js/daterangepicker/daterangepicker.js')}}"></script>
<script type="text/javascript">
var table, save_method;
$(function(){
  table = $("#datatable1").DataTable({
          processing: true,
          serverSide: true,
          ajax: '{{ url('pembelianretur_data') }}',
          columns: [
              {data: 'nomor', name: 'nomor'},
              {data: 'kode_retur', name: 'kode_retur'},
              {data: 'nama_barang', name: 'nama_barang'},
              {data: 'nama_gudang', name: 'nama_gudang'},
              {data: 'nama_supplier', name: 'nama_supplier'},
              {data: 'tanggal', name: 'tanggal'},
              {data: 'jumlah', name: 'jumlah'},
              {data: 'aksi', name: 'aksi', orderable: false, searchable: false}
          ],
          "columnDefs": [
            { targets: 0, orderable: false, searchable: false, }
          ],
          createdRow: function(row, data, index){
            $('td', row).eq(6).attr('align','right'); // 6 is index of column
          },
    });

  $('input[name=search_tanggal]').daterangepicker({
   locale: {
      format: 'DD-MM-YYYY',
      separator: "  s.d. ",
    }
  });

    $('[name=popup_barang]').select2({
            placeholder: "--- Pilih ---",
            ajax: {
                url: '{{url('pembelianretur_get_barang')}}',
                dataType: 'json',
                data: function (params) {
                    return {
                        q: $.trim(params.term),
                        //results: data
                        gudang: $("[name=popup_gudang]").val()
                    };
                },
                processResults: function (data) {
                      var results = [];
                      $.each(data, function(index,item){
                        var text_item = item.barang_kode+" || "+item.barang_nama+" || ("+item.barang_alias+")"+" || "+item.jumlah+" "+item.nama_satuan;
                        if(item.barang_alias === null || item.barang_alias === "" || item.barang_alias === 0){
                          text_item = item.barang_kode+" || "+item.barang_nama+" || "+item.jumlah+" "+item.nama_satuan;
                        }
                        results.push({
                          id:item.barang_id,
                          satuan_id:item.satuan_id,
                          satuan_nama:item.satuan_nama,
                          nama:item.barang_nama,
                          harga:item.harga,
                          kode:item.barang_kode,
                          jumlah:item.jumlah,
                          alias:item.barang_alias,
                          text:text_item
                        });
                      });
                      return{
                        results:results
                      };
                },
                cache: true
              }        
  });

    $('[name=popup_penyedia]').select2({
            placeholder: "Pilih...",
            //minimumInputLength: 2,
            ajax: {
                url: '{{url('pembelianretur_get_supplier')}}',
                dataType: 'json',
                data: function (params) {
                    return {
                        q: $.trim(params.term)
                    };
                },
                processResults: function (data) {
                      var results = [];
                      $.each(data, function(index,item){
                        results.push({
                          id:item.supplier_id,
                          nama:item.supplier_nama,
                          alamat:item.supplier_alamat,
                          telp:item.supplier_telp,
                          top:item.supplier_tempo,
                          text:item.supplier_nama,
                        });
                      });
                      return{
                        results:results
                      };
                },
                cache: true
            }   
  }); 

  

});

$("[name=popup_barang]").on('change',function(e){
    var d = $("[name=popup_barang]").select2('data')[0];
    var id_table =  $("[name=popup_id_table]").val();
    if(id_table==""){
    $("[name=popup_satuan]").val(d.satuan_id).trigger('change');
    $("[name=popup_stok]").val(d.jumlah);
    $("#help_popup_stok").text(format_angka(d.jumlah));
    $("[name=popup_jumlah]").attr('max',d.jumlah);
    }
  });


$('[name=popup_jumlah]').on('keyup',function(){
  var jum = $(this).val();
  $('#help_popup_jumlah').text(format_angka(jum));
});

$("[name=popup_gudang]").on('change',function(){
  var id_gudang = $(this).val();
  var id_table = $("[name=popup_id_table]").val();
  if(id_table==""){
  $("[name=popup_barang]").html('<option></option>');
  $('[name=popup_stok]').val('');
  $('[name=popup_fisik]').val('');
  $('[name=popup_selisih]').val('');
  $('#help_popup_stok').text(parseInt(0));
  $('#help_popup_fisik').text(parseInt(0));
  $('#help_popup_selisih').text(parseInt(0));
  }
});


$("#btn_tambah").click(function(){
    $('[name=popup_id_table]').val('');
    $('[name=popup_kode]').val('');
    $('[name=popup_tanggal]').val(null).trigger('change')
    $('[name=popup_barang]').html('<option></option>');
    $('[name=popup_supplier]').html('<option></option>');
    $('[name=popup_jumlah]').val('');
    $('[name=popup_stok]').val('');
    $('[name=popup_satuan]').val('').trigger('change');
    $('#help_popup_jumlah').text(format_angka(parseInt(0)));
    $('#help_popup_stok').text(format_angka(parseInt(0)));
    $('#modal-form').modal('show');
});

function edit(id){
    var id_barang = $("#table_idbarang"+id).val();
    var nama_barang = $("#table_namabarang"+id).val();
    var id_gudang = $("#table_idgudang"+id).val();
    var id_supplier = $("#table_idsupplier"+id).val();
    var nama_supplier = $("#table_namasupplier"+id).val();
    var id_satuan = $("#table_idsatuan"+id).val();

    $("[name=popup_id_table]").val($("#table_id"+id).val());
    var barangOption = new Option(nama_barang, id_barang, true, true);
    $('[name=popup_barang]').append(barangOption).trigger('change');
    var penyediaOption = new Option(nama_supplier, id_supplier, true, true);
    //$("[name=popup_penyedia]").html('<option value="'+id_supplier+'">'+nama_supplier+'</option>');
    $('[name=popup_penyedia]').append(penyediaOption).trigger('change');
    $("[name=popup_gudang]").val(id_gudang).trigger('change');
    $("[name=popup_kode]").val($("#table_kode"+id).val());
    $("[name=popup_tanggal]").val($("#table_tanggal"+id).val()).trigger('change');
    $("[name=popup_ket]").text($("#table_keterangan"+id).val());
    $("[name=popup_jumlah]").val($("#table_jumlah"+id).val());
    $("[name=popup_satuan]").val(id_satuan).trigger('change');
    $("[name=popup_idlog_stok]").val($("#table_idlog_stok"+id).val());
    $("#help_popup_jumlah").text(format_angka($("#table_jumlah"+id).val()));

    $.ajax({
        url: "{{ url('pembelianretur_get_stok') }}",
        type: 'post',
        dataType: 'json',
        headers : {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        data: {id_barang:id_barang,id_gudang:id_gudang,id_satuan:id_satuan},
        success: function(data){
          if(data!=""){
          var stok = parseFloat(data[0]['jumlah']);
          var jumlah = parseFloat(stok)+parseFloat($("#table_jumlah"+id).val());
            $("[name=popup_stok]").val(stok);
            $("#help_popup_stok").text(format_angka(stok));
            $("[name=popup_jumlah]").attr('max',jumlah);
          }else{
            $("[name=popup_stok]").val(0);
            $("#help_popup_stok").text(format_angka(0));
            $("[name=popup_jumlah]").attr('max',$("#table_jumlah"+id).val());
          }
        }
      });


    $('#modal-form').modal('show');
  }

function detail(id){
    var id_barang = $("#table_idbarang"+id).val();
    var nama_barang = $("#table_namabarang"+id).val();
    var id_gudang = $("#table_idgudang"+id).val();
    var nama_gudang = $("#table_namagudang"+id).val();
    var id_supplier = $("#table_idsupplier"+id).val();
    var nama_supplier = $("#table_namasupplier"+id).val();
    var id_satuan = $("#table_idsatuan"+id).val();
    var nama_satuan = $("#table_namasatuan"+id).val();

    $('[name=popup_edit_barang]').val(nama_barang);
    $('[name=popup_edit_penyedia]').val(nama_supplier);
    $("[name=popup_edit_gudang]").val(nama_gudang);
    $("[name=popup_edit_kode]").val($("#table_kode"+id).val());
    $("[name=popup_edit_tanggal]").val($("#table_tanggal"+id).val()).trigger('change');
    $("[name=popup_edit_ket]").text($("#table_keterangan"+id).val());
    $("[name=popup_edit_jumlah]").val($("#table_jumlah"+id).val());
    $("[name=popup_edit_satuan]").val(nama_satuan);
    $("[name=popup_edit_idlog_stok]").val($("#table_idlog_stok"+id).val());
    $("#help_popup_edit_jumlah").text(format_angka($("#table_jumlah"+id).val()));

    $.ajax({
        url: "{{ url('pembelianretur_get_stok') }}",
        type: 'post',
        dataType: 'json',
        headers : {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        data: {id_barang:id_barang,id_gudang:id_gudang,id_satuan:id_satuan},
        success: function(data){
          if(data!=""){
          var stok = parseFloat(data[0]['jumlah']);
          var jumlah = parseFloat(stok)+parseFloat($("#table_jumlah"+id).val());
            $("[name=popup_edit_stok]").val(stok);
            $("#help_popup_edit_stok").text(format_angka(stok));
            $("[name=popup_edit_jumlah]").attr('max',jumlah);
          }else{
            $("[name=popup_edit_stok]").val(0);
            $("#help_popup_edit_stok").text(format_angka(0));
            $("[name=popup_edit_jumlah]").attr('max',$("#table_jumlah"+id).val());
          }
        }
      });

    $('#modal-form_detail').modal('show');
  }


$("#btn_popup_simpan").click(function(){
    var id = $("[name=popup_id_table]").val();
    var id_barang = $("[name=popup_barang]").val();
    var jumlah = $("[name=popup_jumlah]").val();
    var satuan = $("[name=popup_satuan]").val();
    var id_gudang = $("[name=popup_gudang]").val();
    var id_supplier = $("[name=popup_supplier]").val();
    var tanggal = $("[name=popup_tanggal]").val();
    if(id_barang != '' && jumlah != '' && satuan != '' && id_gudang != '' && id_supplier != '' && tanggal != ''){
      $.ajax({
        url: "{{ url('pembelianretur_simpan') }}",
        type: 'post',
        dataType: 'json',
        headers : {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        data: $("#form_retur").serialize(),
        success: function(respon){
          table.ajax.reload();
          $('#modal-form').modal('hide');
        }
      })
    }
  })


function deleteData(id){
  if(confirm("Apakah yakin data akan dihapus?")){
    $.ajax({
      url : "pembelianretur_hapus",
      type : "POST",
      data: {id : id},
      headers : {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
          },
      success : function(data){
        table.ajax.reload();
      },
      error : function(){
        alert("Tidak dapat menghapus data!");
      }
    });
  }
}

$(".btn-search-tanggal").on("click",function(){
    var tanggal = $("[name=search_tanggal]").val();
    var url = '<?=$hal?>_searchtanggal';
    $('#datatable1').DataTable().clear();
    $('#datatable1').DataTable().destroy();
    table_search = $('#datatable1').DataTable({
    processing: true,
    serverSide: true,
    "ajax" : {
      "url" : "{{url('pembelianretur_searchtanggal')}}",
      "data" : {tanggal:tanggal}
    },
    columns: [
              {data: 'nomor', name: 'nomor'},
              {data: 'kode_retur', name: 'kode_retur'},
              {data: 'nama_barang', name: 'nama_barang'},
              {data: 'nama_gudang', name: 'nama_gudang'},
              {data: 'nama_supplier', name: 'nama_supplier'},
              {data: 'tanggal', name: 'tanggal'},
              {data: 'jumlah', name: 'jumlah'},
              {data: 'aksi', name: 'aksi', orderable: false, searchable: false}
          ],
          "columnDefs": [
            { targets: 0, orderable: false, searchable: false, }
          ],
          createdRow: function(row, data, index){
            $('td', row).eq(6).attr('align','right'); // 6 is index of column
          },
  });
});

$(".btn-reset-tanggal").on("click",function(){
    $('#datatable1').DataTable().clear();
    $('#datatable1').DataTable().destroy();
    table = $("#datatable1").DataTable({
          processing: true,
          serverSide: true,
          ajax: '{{ url('pembelianretur_data') }}',
          columns: [
              {data: 'nomor', name: 'nomor'},
              {data: 'kode_retur', name: 'kode_retur'},
              {data: 'nama_barang', name: 'nama_barang'},
              {data: 'nama_gudang', name: 'nama_gudang'},
              {data: 'nama_supplier', name: 'nama_supplier'},
              {data: 'tanggal', name: 'tanggal'},
              {data: 'jumlah', name: 'jumlah'},
              {data: 'aksi', name: 'aksi', orderable: false, searchable: false}
          ],
          "columnDefs": [
            { targets: 0, orderable: false, searchable: false, }
          ],
          createdRow: function(row, data, index){
            $('td', row).eq(6).attr('align','right'); // 6 is index of column
          },
    });
});
</script>

@endsection
