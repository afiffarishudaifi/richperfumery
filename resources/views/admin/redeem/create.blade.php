  ...
  <div class="row">
    <div class="col-md-12">
      <div class="box box-primary">
        ...
        <!-- /.box-header -->
        <form role="form" class="form_belanja" action="{{url('redeem_simpan')}}" method="POST">
          ...
          <div class="box-body">
            ...
            <div class="row">
              <div class="col-md-6">
                ...
                <div class="form-group">
                  ...
                  <input class="form-control datepicker" type="text" name="tanggal_tempo" placeholder="Tanggal Tempo" value="{{($data['data']['tanggal_tempo'] == '') ? date('d-m-Y'):$data['data']['tanggal_tempo']}}" readonly="" required/>
                </div>
                <div class="form-group">
                  <label for="exampleInputPassword1">Nama Pelanggan</label>
                  <select name="id_pelanggan" id="id_pelanggan" class="form-control selectize" style="width: 100%;" required="" onKeyup="get_pelanggan($(this))" onChange="get_pelanggan($(this))" onKeypress="get_pelanggan($(this))">
            ...
            <div class="form-group col-md-6 col-sm-12 m-l-5" id="div_form_pembayaran">
              <div class="table-responsive">
               <table class="table table-bordered table-form table-totalbayar">
                <tbody>
                  <tr class="active">
                    <!-- <td>Paper Bag</td> -->
                    <td>Total Tukar Poin</td>
                    <td class="text-right">
                 ...
                 <tr id="div_poin">
                    <td width="30%">Total Poin Transaksi</td>
                    <td><input type="hidden" name="td_poin" value="0"><p class="help_block pull-right" id="td_poin">0 Poin</td>
                 </tr> 
                  <tr>
          ...
          <div class="box-footer">
            <a href="{{url('redeem')}}" class="btn btn-md btn-warning"><span class="glyphicon glyphicon-arrow-left"></span> Kembali</a>
            <button type="submit" class="btn bg-blue btn-md pull-right btn_simpan"><span class="glyphicon glyphicon-floppy-disk"></span> Simpan</button>
          </div>
        </form>
  </div>
</div>
...
$("#btn_tambah").click(function(){
    ...
    $('[name=popup_status_redeem]').val('1');
...
$("[name=popup_harga]").on('keyup', function(){
  ...
  $("[name=popup_harga_hidden]").val(harga);
  if(jumlah > 0){
  get_jum();
  }
});
...
$("[name=id_pelanggan]").on('change',function(){
  ...
  $.ajax({
      ...
      success: function(respon){
          ...
          var poin_tabel = parseFloat(0);
          $('input[name^="tabel_total_poin_produk"]').each(function(){
            poin_tabel = parseFloat($(this).val());
            poin_tabel += parseFloat(poin_tabel);
          })
          if(id != ''){
            $("#div_poinku").show();
            $("[name=poin]").val(parseInt(0));
            $("[name=poin_tabel]").val(parseFloat(0));
            if(poin > 0){
              if(id_kasir == ''){
...
$("#btn_popup_simpan").click(function(){
  ...
  if(id_data == ''){
    ...
    table_barang.row.add(['<div><center></center></div>',
          ...
          '<input type="hidden" name="tabel_total_poin_produk[]" id="tabel_total_poin_produk'+tb_no+'" value="'+total_poin+'">'
    ]).draw(false);
    // get_subtotal();
    get_poinsubtotal();
    }else{
      ...
      alert('poin anda kurang '+kurang_poin+' Poin');
        ...
        $("#list_totalpoin"+id_data).text(format_angka(total_poin,0)+" Poin");
        ...
        if(respon.produk.length > 0){
          for(i in respon.produk){
            table_barang.row.add(['<div><center></center></div>',
              ...
              '<div><button type="button" class="btn btn-xs btn-warning" onclick="edit_table('+respon.produk[i].id+')"><i class="fa fa-edit"></i> </button></button><button class="btn btn-xs btn-danger" type="button"  onclick="hapus($(this),'+respon.produk[i].id+')""><i class="fa fa-trash"></i></button></div>'+
...
function get_subtotal(this_){
  ...
  // $('input[id^="tabel_total_poin_produk"]').each(function(){
  ...
  console.log(poin, pelanggan_poin);

  if(id_kasir == ''){
    pelanggan_kurang_poin = parseFloat(pelanggan_poin-poin);
  }else{
    
    // pelanggan_kurang_poin_lama = parseFloat(pelanggan_poin+poin)
    // pelanggan_kurang_poin = parseFloat(pelanggan_kurang_poin_lama-poin);
    pelanggan_kurang_poin = parseFloat(pelanggan_poin);
    console.log(pelanggan_kurang_poin);
  }
...
function netto_poin(id){
  var poin = parseFloat($("#tabel_total_poin_produk"+id).val());
  var pelanggan_poin = parseFloat($("[name=poin]").val());
  var d_poin = parseFloat(parseFloat(poin)+parseFloat(pelanggan_poin));
  console.log(id, poin, pelanggan_poin, d_poin);
...
function get_pelanggan(this_=''){
  ...
  $.ajax({
      ...
      success: function(respon){
          /*if(respon.length > 0){
            $('[name=id_pelanggan]').html('');
            $('[name=id_pelanggan]').append('<option value=""> -- Pilih -- </option>');
            for(i in respon){
              var selected = "";
              if(this_ != ''){
                var id = this_.val();
              ...
              $("[name=id_pelanggan]").append('<option value="'+respon[i].id_pelanggan+'" '+selected+'>'+respon[i].nama_pelanggan+'</option>');
            }
          }*/
          ...
          if(respon.length > 0){
            ...
            for(i in respon){
              ...
              // select.val(id_pelanggan).trigger('change');
            }
          }
