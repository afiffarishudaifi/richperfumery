...
$("[name=popup_jumlah]").on('keyup', function(){
  ...
  if (harga_promo > 0 && jumlah > 1){
    $('[name=popup_promo]').val('YA').trigger('change');
    $(`#wrapper-promo`).show()
  }else{
    $(`#wrapper-promo`).hide()
  }
...
$("#btn_popup_simpan").click(function(){
  ...
  if(status == '1'){
    ...
    if ((pilih_promo) && pilih_promo == 'YA') {
      ...
      // promo = 0 INI DIKOMEN KARENA BUTUH NOMINAL HARGA PROMO SETELAH EDIT.
    }
  ...
  if(id_data == ''){
    ...
    table_barang.row.add(['<div><center></center></div>',
      ...
      if(status == '1'){
        ...
        $("#list_total"+id_data).html(accounting.formatMoney(total)+total_lama);
