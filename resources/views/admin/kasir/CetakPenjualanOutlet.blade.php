...
class Pdf extends PDF_MC_Table{
	...
	function Header(){
	    if($this->PageNo() == 1){		
            ...
            if($this->PageNo() != 0){   
                ...
                $this->Row_draw(array('Jml','Nama Produk','Harga @ Rp.','Total @ Rp.'));
	...
	function Content()
	{
	    ...
	    if(count($this->barang) > 0){
			foreach($this->barang as $r){
		        $this->row(array($r->jumlah." ".$r->inisial_satuan, $r->nama_barang, format_angka($r->harga), format_angka($r->total)));
		    }
		}
        ...
        $total_metodebayar  = $this->data->total_metodebayar;
        if($this->data->carabayar==1 && ($this->data->metodebayar2==null||$this->data->metodebayar2=="")){
            $d_metodebayar = $this->data->total_tagihan-($this->data->total_potongan-$this->data->ongkos_kirim)-$this->data->uang_muka;
            $total_metodebayar  = $d_metodebayar;
        }

        $this->SetWidths(array($this->width_p*60/100, $this->width_p*17/100));
        $this->SetAligns(array('L', 'R'));
        $this->Row_noborder(array('',$this->data->nama_metodebayar));
        $this->setXY($this->GetX()+$this->width_p*77/100,$this->GetY()-5);
        $this->SetWidths(array($this->width_p*17/100));
        $this->SetAligns(array('R'));
        $this->Row(array(format_angka($total_metodebayar)));

        $total_metodebayar2 = 0;
        if($this->data->metodebayar2!=null||$this->data->metodebayar2!=""){
        $this->SetWidths(array($this->width_p*60/100, $this->width_p*17/100));
        $this->SetAligns(array('L', 'R'));
        $this->Row_noborder(array('',$this->data->nama_metodebayar2));
        $this->setXY($this->GetX()+$this->width_p*77/100,$this->GetY()-5);
        $this->SetWidths(array($this->width_p*17/100));
        $this->SetAligns(array('R'));
        $this->Row(array(format_angka($this->data->total_metodebayar2)));
        $total_metodebayar2 = $this->data->total_metodebayar2;
        }

        $total_tagihan = $this->data->total_tagihan-($this->data->total_potongan-$this->data->ongkos_kirim)-$this->data->uang_muka-($total_metodebayar+$total_metodebayar2);

        $this->SetWidths(array($this->width_p*60/100, $this->width_p*17/100));
        $this->SetAligns(array('L', 'R'));
        $this->Row_noborder(array('','Tagihan Rp.'));
        $this->setXY($this->GetX()+$this->width_p*77/100,$this->GetY()-5);
        $this->SetWidths(array($this->width_p*17/100));
        $this->SetAligns(array('R'));
        $this->Row(array(format_angka($this->data->uang_muka)));
