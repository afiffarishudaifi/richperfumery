...
$("[name=popup_barang]").on('change', function(){
  ...
  // $("[name=popup_stok]").val(d.jumlah_stok);
...
function cek_stok(){
  ...
  $.ajax({
      url: "{{ url('penjualangrosir_get_stok')}} ",
      type: 'post',
      data: {id_barang : id_barang, id_satuan : id_satuan, id_gudang : id_gudang},
      headers : {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
      success: function(respon){
        console.log(respon);
        $("[name=popup_stok]").val(respon);
      }
  });
}
