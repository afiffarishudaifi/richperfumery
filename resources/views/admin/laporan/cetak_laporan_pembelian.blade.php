...
class Pdf extends PDF_MC_Table{
	...
	function Header(){
		...
		if($this->PageNo() != 0){
			...
			$this->SetWidths(array(10, 20, 65, 25, 27.5, 27.5, 25));
	        $this->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C', 'C'));
	        $this->Row(array("NO.", "KODE BARANG", "NAMA PARFUM", "STOCK AWAL", "PEMBELIAN HARI INI", "RETUR HARI INI", "STOK AKHIR"));
	...
	function Content(){
        ...
        if(count($this->data) > 0){
        	foreach($this->data as $k){
        		$this->Row(array($no, $k->barang_kode, $k->barang_nama, format_angka($k->jumlah_stokawal)." ".$k->satuan_satuan, format_angka($k->jumlah_pembelian)." ".$k->satuan_satuan, format_angka($k->jumlah_retur)." ".$k->satuan_satuan, format_angka($k->jumlah_terakhir)." ".$k->satuan_satuan ));
