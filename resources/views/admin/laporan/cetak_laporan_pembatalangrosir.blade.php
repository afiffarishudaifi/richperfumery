...
class Pdf extends PDF_MC_Table{
	...
	function Header(){
		...
		if($this->PageNo() != 0){
			...
			$this->SetWidths(array(10, 25, 65, 25, 25, 26, 25));
	        $this->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C', 'C'));
	        $this->Row(array("NO.", "KODE BARANG", "NAMA BARANG", "STOCK AWAL", "PEMBATALAN HARI INI", "STOK AKHIR", "HARGA"));
	...
	function Content(){
        ...
        if(count($this->data) > 0){
        	foreach($this->data as $k){
        		$this->Row(array($no, $k->barang_kode, $k->barang_nama, format_angka($k->jumlah_stokawal)." ".$k->satuan_satuan, format_angka($k->jumlah_terjual)." ".$k->satuan_satuan, format_angka($k->jumlah_terakhir)." ".$k->satuan_satuan, "Rp. ".format_angka($k->harga_penjualan) ));
