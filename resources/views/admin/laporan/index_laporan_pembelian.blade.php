...
<section class="content col-md-6">
  <div class="box">
    ...
    <div class="box-body">
      <form enctype="" class="form form_filter form_laporanpembelian" target="_blank" action="{{url('laporanpembelian_cetak')}}" autocomplete="off">
          ...
          <div class="col-md-12">
            ...
            <label>Kategori</label>
             <select class="form-control" name="kategori" style="width: 100%;">
               <option value="1">Rekap Stok</option>
               <option value="2">Per-Nota</option>
               <option value="3">Per-Barang</option>
             </select>
            </div>
          </div>
...
$(document).ready(function() {
  ...
  $('[name=barang]').select2({
            placeholder: "--- Pilih ---",
            ajax: {
                url: '{{url('laporanpembelian_get_barang')}}',
                dataType: 'json',
 ...
 $('#btn_excel').validator().on('click', function(e){
    if(!e.isDefaultPrevented()){
      ...
      if(kategori == 3){
        ...
        var where = [gudang,tanggal,tanggal2,kategori];
      }
      var url_where = where.join("/");
      window.open("{{url('laporanpembelian_excel')}}/"+url_where,'_blank');
 ...
 $('#btn_filter').on('click',function(e){
    if(!e.isDefaultPrevented()){
      ...
      var url_where = where.join("/");    
      window.open("{{url('laporanpembelian_hasil')}}/"+url_where,'_blank');      
    }
 })
