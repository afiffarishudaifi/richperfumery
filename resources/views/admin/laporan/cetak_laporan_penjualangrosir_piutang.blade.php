...
class Pdf extends PDF_MC_Table{
	...
	public $def_width = 310, $def_height = 210;
	...
	function Header(){
		...
		if($this->PageNo() != 0){
			...
			$this->SetWidths(array(10, 30, 30, 45, 75, 30, 50, 25));
            $this->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C', 'C', 'C'));
            $this->Row_bawahhide(array("NO.", "TANGGAL", "TANGGAL", "NOMER", "NAMA", "TOTAL", "KET.", ""));
            $this->Row_atasputus(array("", "JUAL", "BAYAR", "NOTA", "CUSTOMER", "JUAL", "", ""));

	        $this->setFont('arial','',8);
            $this->SetWidths(array(10, 30, 30, 45, 75, 50, 30, 25));
            $this->SetAligns(array('C', 'C', 'C', 'L', 'L', 'R', 'L', 'C'));
	...
	function Content(){
        ...
        if(count($this->data) > 0){
            foreach($this->data as $k){
                ...
                $this->Row(array($no, tgl_full($k->tanggal,'0'), tgl_full($k->tanggal_bayar,'0'), strtoupper($k->no_faktur), ucwords($pelanggan), 'Rp. '.format_angka($k->total), $k->keterangan, $k->status));
...
$Pdf->Content();
$Pdf->SetTitle("Cetak Laporan Penjualan Piutang - (".$data['tanggal_awal']."-".$data['tanggal_akhir'].")");
$Pdf->Output("Cetak Laporan Penjualan Piutang - (".$data['tanggal_awal']."-".$data['tanggal_akhir'].").pdf", "I");
