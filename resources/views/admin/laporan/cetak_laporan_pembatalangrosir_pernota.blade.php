...
class Pdf extends PDF_MC_Table{
	...
	function Header(){
		...
		if($this->PageNo() != 0){
			...
			$this->SetWidths(array(10, 30, 55, 45, 60, 25, 25, 25, 25));
	        $this->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C','C'));
	        $this->Row_bawahhide(array("NO.", "NOMOR", "NAMA", "NAMA", "NAMA", "JUMLAH", "TOTAL", "DIBATALKAN", "TANGGAL"));
            $this->Row_atasputus(array("", "NOTA", "PELANGGAN", "GUDANG", "PRODUK", "", "", "OLEH","BATAL"));
	...
	function Content(){
        ...
        if(count($this->data) > 0){
        	foreach($this->data as $k){
                ...
                $this->Row(array($no,  
                                strtoupper($k['no_faktur']), 
                                ucwords($k['pelanggan']), 
                                ucwords($k['gudang']), 
                                ucwords($k['nama_produk']), 
                                format_angka($k['jumlah']),
                                'Rp. '.format_angka($k['total']), 
                                $k['dibatalkan'], 
