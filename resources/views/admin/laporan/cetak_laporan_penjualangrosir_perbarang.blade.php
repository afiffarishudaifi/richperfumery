...
class Pdf extends PDF_MC_Table{
	...
	function Header(){
		...
		if($this->PageNo() != 0){
			...
			$this->SetWidths(array(10, 25, 35, 80, 35, 20, 30, 30, 30));
	        $this->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C'));
	        $this->Row_bawahhide(array("NO.", "TANGGAL", "NOMER", "NAMA", "CARA", "JUMLAH", "HARGA", "TOTAL", "KET."));
            $this->Row_atasputus(array("", "JUAL", "NOTA", "PARFUM", "BAYAR", "", "", "", ""));

	        $this->setFont('arial','',8);
			$this->SetWidths(array(10, 25, 35, 80, 35, 20, 30, 30, 30));
	        $this->SetAligns(array('C', 'C', 'L', 'L', 'L', 'R', 'R', 'R'));
	...
	function Content(){
        ...
        $this->setFont('arial','',8);
		$this->SetWidths(array(10, 25, 35, 80, 35, 20, 30, 30, 30));
        $this->SetAligns(array('C', 'C', 'L', 'L', 'L', 'R', 'R', 'R', 'L'));
        ...
        if(count($this->data) > 0){
        	foreach($this->data as $k){
        		...
        		$this->Row(array($no, tgl_full($k->tanggal,'0'), strtoupper($k->no_faktur), ucwords($k->nama_produk), strtoupper($k->metodebayar), format_angka($k->jumlah), 'Rp. '.format_angka($k->harga), 'Rp. '.format_angka($k->total), $k->keterangan));
...
$Pdf->Content();
$Pdf->SetTitle("Cetak Laporan Penjualan Perbarang - (".$data['tanggal_awal']."-".$data['tanggal_akhir'].")");
$Pdf->Output("Cetak Laporan Penjualan Perbarang - (".$data['tanggal_awal']."-".$data['tanggal_akhir'].").pdf", "I");
