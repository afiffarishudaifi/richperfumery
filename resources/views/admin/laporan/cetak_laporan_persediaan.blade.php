...
class Pdf extends PDF_MC_Table{
	...
	function Header(){
		...
		if($this->PageNo() != 0){
			...
			$this->SetWidths(array(10, 25, 100, 45, 25, 30, 30, 30));
	        $this->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C', 'C', 'C'));
	        $this->Row(array("NO.", "KODE BARANG", "NAMA PARFUM", "NAMA GUDANG", "STOCK AWAL", "PEMBELIAN HARI INI", "RETUR HARI INI", "STOK AKHIR"));

	        $this->setFont('arial','',8);
			$this->SetWidths(array(10, 25, 100, 45, 25, 30, 30, 30));
	        $this->SetAligns(array('C', 'C', 'L', 'L', 'R', 'R', 'R', 'R'));
	...
	function Content(){
        ...
        if(count($this->data) > 0){
        	foreach($this->data as $k){
        		$this->Row(array($no, $k->barang_kode, $k->barang_nama, $k->gudang_nama, format_angka($k->jumlah_stokawal)." ".$k->satuan_satuan, format_angka($k->jumlah_masuk)." ".$k->satuan_satuan, format_angka($k->jumlah_keluar)." ".$k->satuan_satuan, format_angka($k->jumlah_terakhir)." ".$k->satuan_satuan ));
...
$Pdf->Content();
$Pdf->SetTitle("Cetak Laporan Persediaan - (".$data['tanggal_awal']."-".$data['tanggal_akhir'].")");
$Pdf->Output("Cetak Laporan Persediaan - (".$data['tanggal_awal']."-".$data['tanggal_akhir'].").pdf", "I");
