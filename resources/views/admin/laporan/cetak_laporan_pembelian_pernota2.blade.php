...
class Pdf extends PDF_MC_Table{
	...
	function Header(){
		...
		if($this->PageNo() != 0){
			...
			$this->SetWidths(array(10, 30, 50, 30, 30, 65, 20, 30, 30));
	        $this->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C'));
	        $this->Row_bawahhide(array("NO.", "TANGGAL", "NAMA", "NOMER", "KODE", "NAMA", "JUMLAH", "HARGA", "TOTAL"));
            $this->Row_atasputus(array("", "BELI", "SUPPLIER", "NOTA", "BARANG", "BARANG", "", "", ""));

	        $this->setFont('arial','',8);
			$this->SetWidths(array(10, 30, 50, 30, 30, 65, 20, 30, 30));
	        $this->SetAligns(array('C', 'C', 'L', 'L', 'L', 'L', 'R', 'R', 'R'));
	...
	function Content(){
        ...
        if(count($this->data) > 0){
        	foreach($this->data as $k){
        		$this->Row(array($no, tgl_full($k->tanggal,'0'), strtoupper($k->nama_supplier), strtoupper($k->no_faktur),strtoupper($k->kode_barang), ucwords($k->nama_barang), format_angka($k->jumlah).' '.$k->nama_satuan, 'Rp. '.format_angka($k->harga), 'Rp. '.format_angka($k->total)));
